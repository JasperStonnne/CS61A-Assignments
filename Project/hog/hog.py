...
def boar_brawl(player_score, opponent_score):
    ...
    return max(1, 3 * abs(opponent_ones - player_tens))
    # END PROBLEM 2
...
def make_averaged(original_function, times_called=1000):
    ...
    # BEGIN PROBLEM 8
    def averaged(*args):
        total = sum(original_function(*args) for _ in range(times_called))
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    best_rolls, best_avg = 1, 0
    for n in range(1, 11):
        avg = averaged_roll(n, dice)
        if avg > best_avg:
            best_avg = avg
            best_rolls = n
    return best_rolls
